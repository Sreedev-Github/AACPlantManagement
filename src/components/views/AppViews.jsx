...
import { convertPiecesToCbm, getVolumePerPieceFromSize } from '../../utils/appHelpers';
...
export const HomeView = ({ role, stats, setViewAndFilter, getTodayString, setView, setProductionTab, setViewDate, setEditingOrderId, setFormData, SIZES }) => (
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between bg-white p-3 rounded-xl shadow-sm border border-slate-200">
        ...
        <button onClick={handleDownloadReport} className="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-lg text-sm font-bold flex items-center shadow-sm"><Download className="w-4 h-4 mr-1.5" /> Download {productionTab === 'raw-material' ? 'RM Stock' : 'Prod. Report'}</button>
                ...
                <tbody className="divide-y divide-slate-100">
                  {rawData.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      ...
                      <td className="px-2 py-2 text-right font-bold text-slate-700 bg-slate-50/30">{item.opening.toFixed(2)}</td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.receipt} onUpdate={(v) => updateRawStock(idx, 'receipt', v)} tableId="raw-material" rowIndex={idx} colIndex={0} /></td>
                      <td className="px-2 py-2 text-right font-bold text-blue-700 bg-blue-50/30">{item.total.toFixed(2)}</td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.issue} onUpdate={(v) => updateRawStock(idx, 'issue', v)} tableId="raw-material" rowIndex={idx} colIndex={1} /></td>
                      <td className="px-2 py-2 text-right font-bold text-green-700 bg-green-50/30">{item.closing.toFixed(2)}</td>
                      <td className="px-2 py-2"><EditableCell type="text" value={item.remarks} className="text-left font-bold text-slate-700 justify-start whitespace-normal break-words" onUpdate={(v) => updateRawStock(idx, 'remarks', v)} tableId="raw-material" rowIndex={idx} colIndex={2} /></td>
                    </tr>
                  ))}
                  <tr className="bg-slate-100 font-bold text-slate-900">
                    <td colSpan="3" className="px-2 py-3 text-right text-[11px] uppercase">Total</td>
                    <td className="px-2 py-3 text-right bg-blue-100/50">{calculateColumnTotal(rawData, 'opening')}</td>
                ...
                <tbody className="divide-y divide-slate-100">
                  {finishedData.items.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      ...
                      <td className="px-2 py-2 text-right font-mono text-slate-600 bg-slate-50/30">{item.opening.toFixed(2)}</td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.segregation} onUpdate={(v) => updateFinishedStock(idx, 'segregation', v)} tableId="finished-stock" rowIndex={idx} colIndex={0} /></td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.sale} onUpdate={(v) => updateFinishedStock(idx, 'sale', v)} tableId="finished-stock" rowIndex={idx} colIndex={1} /></td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.proRejection} onUpdate={(v) => updateFinishedStock(idx, 'proRejection', v)} tableId="finished-stock" rowIndex={idx} colIndex={2} /></td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.loadingRejection} onUpdate={(v) => updateFinishedStock(idx, 'loadingRejection', v)} tableId="finished-stock" rowIndex={idx} colIndex={3} /></td>
                      <td className="px-2 py-2"><EditableCell type="text" inputMode="decimal" className="text-right font-bold text-slate-700" value={item.selfUse} onUpdate={(v) => updateFinishedStock(idx, 'selfUse', v)} tableId="finished-stock" rowIndex={idx} colIndex={4} /></td>
                  ...
                  <tr className="bg-slate-200 font-bold text-slate-900 border-t-2 border-slate-300">
                    ...
                    <td className="px-2 py-3 text-right bg-blue-200/50">{calculateColumnTotal(finishedData.items, 'opening')}</td>
                  ...
                  <tr className="bg-yellow-50 font-bold border-t border-yellow-200">
                    ...
                    <td className="px-2 py-3 text-right font-mono text-slate-600 bg-yellow-100/50">{finishedData.mortarBag.opening}</td>
                    <td className="px-2 py-3"><EditableCell type="text" inputMode="decimal" className="text-right bg-white/50 font-bold text-slate-700" value={finishedData.mortarBag.receipt} onUpdate={(v) => updateMortarBag('receipt', v)} tableId="finished-stock" rowIndex={FINISHED_STOCK_SIZES.length} colIndex={0} /></td>
                    <td className="px-2 py-3"><EditableCell type="text" inputMode="decimal" className="text-right bg-white/50 font-bold text-slate-700" value={finishedData.mortarBag.sale} onUpdate={(v) => updateMortarBag('sale', v)} tableId="finished-stock" rowIndex={FINISHED_STOCK_SIZES.length} colIndex={1} /></td>
...
const VEHICLE_OPTIONS = {
  MH12AB1234: { vehicleType: '12 Wheeler', transporter: 'ABC Logistics' },
  MH15CD5678: { vehicleType: '10 Wheeler', transporter: 'Shree Transport' },
  GJ05EF9012: { vehicleType: '14 Wheeler', transporter: 'Swift Carriers' },
};
...
export const NewOrderView = ({ setView, editingOrderId, viewDate, formatDateDisplay, handleSubmitOrder, formData, setFormData, SIZES }) => {
  const clientOptions = Object.keys(CLIENT_SITE_OPTIONS);
  const selectedClient = CLIENT_SITE_OPTIONS[formData.client] || null;
  const locationOptions = selectedClient ? selectedClient.sites.map((site) => site.name) : [];
  ...
  const handleClientChange = (e) => {
    ...
    setFormData((prev) => {
      const clientConfig = CLIENT_SITE_OPTIONS[nextClient];
      const validLocation = clientConfig?.sites.some((site) => site.name === prev.location) ? prev.location : '';
      const matchedSite = clientConfig?.sites.find((site) => site.name === validLocation);

      return {
        ...prev,
        client: nextClient,
        location: validLocation,
        gstin: prev.gstin || clientConfig?.gstin || '',
        rate: prev.aacRateManualOverride ? prev.rate : (matchedSite ? String(matchedSite.aacRate) : ''),
        bjmRate: prev.bjmRateManualOverride ? prev.bjmRate : (matchedSite ? String(matchedSite.bjmRate) : ''),
      };
    });
  };
  ...
  const handleLocationChange = (e) => {
    ...
    setFormData((prev) => {
      const matchedSite = CLIENT_SITE_OPTIONS[prev.client]?.sites.find((site) => site.name === nextLocation);
      return {
        ...prev,
  ...
  const handleVehicleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      return {
        ...prev,
        vehicle: nextVehicle,
        vehicleType: matchedVehicle ? matchedVehicle.vehicleType : prev.vehicleType,
            ...
            {additionalProducts.length > 0 && (
              <div className="space-y-3 pt-1">
                {additionalProducts.map((product, index) => {
                  ...
                  const productPerPiece = getSizeCbmPerPiece(product.size || SIZES[3]);
        ...
        {displayedOrders.map((order) => (
          <Card key={order.id} className="p-5 flex flex-col md:flex-row md:items-start gap-5 hover:shadow-md transition-shadow relative">
            ...
            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 relative">
              ...
              <div><p className="text-xs text-slate-400 uppercase font-bold">Vehicle</p><p className="font-semibold text-slate-700 bg-slate-100 inline-block px-2 rounded text-sm">{order.vehicle}</p>{order.truckType && <span className="ml-2 text-[10px] font-bold text-slate-500 bg-slate-200 px-1.5 rounded">{order.truckType}W</span>}{order.transporter && <p className="text-[10px] text-slate-400 mt-1 truncate max-w-[120px]">{order.transporter}</p>}</div>
